...
| 3 | menu modeselect/menuredraw; map mapscroll | SCENE (0Dh) | full frame (raises `FaithfulVisualGap`) | scene compose | **scene gap** | `render_scene`/`SceneState` not wired into `render_visual`; menu bg present + `draw_string` are runtime-replaced but no mirror SceneState reader | build a `SceneState` reader + wire `render_scene`/`draw_string` through `render_visual` | scene gap | OPEN (located, scene_island.md) |
