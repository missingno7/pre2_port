...
- **DEFERRED (still ASM):** the classifier `4232` — the **ASM producer** of the
...
| `1030:3054` | **panel/HUD copy** — screen-to-screen (`ds=es=A000`) copy via `[0x2DD2]/[0x2DD4]`, 0x14-wide × 0xB0-tall band stepping `di` by 4 (calls `452F`/`451F`/`44C1`) | GUESS | (draw) | — | exact purpose (HUD vs split-screen) |
