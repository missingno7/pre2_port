...
| Location | Name | Confidence | Role | Coverage | Known unknowns |
|---|---|---|---|---|---|
| `1030:107B` | `sqz_decompress` — decompressor public entry (opens file at `1A13:DX`, dispatches, decodes, returns `ax`=out seg) | VERIFIED | replacement | `pre2/checkpoints/`; `--verify-hooks`; `tests/test_sqz_codec.py` | — |
...
- **`1030:6861..6873` — `apply_velocity`. VERIFIED + LIVE-HOOKED** (`pre2/recovered/object_update.py`,
  checkpoint `pre2/checkpoints/object_update.py`, `tests/test_object_update.py`). `Y += sar([+0xA],4)`;
  `[+8]!=0xFFFF` → `X += sar([+8],4)` (12.4 fixed, signed, wrap). Shadow-proven 770/770 + 453/453 exact vs ASM
  over two demos (moving + static); standard verify-mode lockstep 0 divergence. X-vel `-1` collides with the
...
### Secondary-entity update pass (the main-loop spine 1030:021A..022C — recovery started 2026-06-29)
The main loop runs four small fixed-stride (0x12) slot-array updaters back to back, each ticking one
lightweight list (the *emitters* are already recovered: combat_interaction 0x50A8/0x5450, player 0x4F2E):

| routine | list | role | status |
|---|---|---|---|
...
| `1030:60FE` | particles `0x50A8` (32 slots) | physics: velocity `>>4`, gravity Yvel+9 cap 0x100, X-bounce 0/0x1000, tile-collide via `[0x2DDA]`+`[0x7F5E]`/`[0x7E5E]`, sprite anim 0x46-0x49 | OBSERVED (next leaf) |
...
| combat/pickup interaction `88D7` | `1030:88D7` (orchestrator) | **MAPPED (Stage 0)** — `docs/pre2/combat_interaction_island.md`. Player + thrown-weapon (`0x4F2E`) vs enemies (`8C21`, damage/kill) + bonus tiles (`899E`, pickup/score). Leaves: `pack_spawn_pos` (8BF6) VERIFIED, `roll_bonus_sprite_id` (8C13) ASM_MATCHED (`recovered/combat_interaction.py`). Keystone `8D7B` sprite-hitbox proximity **VERIFIED** (`hitbox_overlap`; 1895 shadow calls/6 demos 0 mismatch; tables `0x7190/0x7191/0x752A` indexed by `(id&0x1FFF)*2`). WHOLE island state logic recovered byte-exact: `8C21` `projectile_vs_enemies` (enemy damage; composes hitbox_overlap+death_handler+play_sfx) + `899E` `bonus_pickup_scan` (bonus pickup; composes 8A5A bonus_hit_handler + 8B6E bonus_collect_tail + flood-fill). |
| `projectile_vs_enemies` | `1030:8C21` | **LIVE HOOK** (`checkpoints/object_interaction.py`; the tiny 88D7 loop stays ASM, calls this + 899E per projectile + player). Live runs native (66 fires/demo), verify-hooks 0 div across kills 115215/140619 + new enemy 190645. |
| `bonus_pickup_scan` | `1030:899E` | **LIVE HOOK** (`checkpoints/object_interaction.py`; bridge `bridge/object_interaction.py`). Applies DS + level-map (es=[0x2DDA]) writes + the on-screen tile re-blit (`screen_di`=0x280*(row%12)+2*(col%20) + recovered `blit_sprite`; 453B is just EGA write-mode-1 setup). Live runs native; verify-hooks 0 div + full-verify clean (120536). |
| `_emit_sfx` dx fix | `checkpoints/player.py` | play_sfx (0x282) indexes by the FULL dx word (digital path bx=dx → [bx*4+0x1009] → [0x1004]/[0x1006] audio state); _emit_sfx now sets the whole dx (dh=0), not just dl. Fixed a latent kill/attack SFX wrong-index → full-verify [0x1004] now byte-exact on kills (115215). |
