"""Tests for the [0x6BC5]!=0 scripted-pose / flying FSM mode (1030:596A + cs:[0x7D6F]).

Byte-exact vs ASM proven offline against two crash-dump oracles from demo_pre2_20260626_105310
(artifacts/flying_witness/ + scratchpad verify_flying*.py): player_fsm_step reproduces both the
...
def test_flying_hold_idle_satdecs_counter():
...
def test_flying_hold_up_decrements_and_kicks_yvel():
...
def test_flying_jump_runs_body_below_threshold():
    ...
    out = _flying_jump(rb, rw)
    assert out[0x4F27] == 2                        # set_anim_b(2) -> state 2
    assert out[0x6BFE] == 0


def test_flying_jump_ends_at_threshold():
...
def test_flying_dispatch_routing():
    rb, rw = _mem({0x6BC8: 0x18, 0x4F1E: 0x200})
    # anim_id 2 -> the flying jump; 8 -> no-op (454C ret)
    assert player_fsm_flying_dispatch(8, rb, rw) == ({}, [])
    out2, _ = player_fsm_flying_dispatch(2, rb, rw)
    assert out2[0x6BC7] == 1                        # routed to _flying_jump
