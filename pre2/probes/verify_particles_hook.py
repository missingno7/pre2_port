"""Whole-state lockstep proof for the particles live replacement (1030:4B8E -> ret 4C2F).

At each 4B8E entry with active particles: snapshot the machine; run the ASM to its ret and capture the
whole state (all memory + regs + the EGA register state); restore; run the recovered ``particles_draw``
replacement; diff. Byte-exact = the recovered pass IS a safe live ASM replacement (draw + Y writeback +
kill + EGA register state). Drives the spider witness (active spider-thread particles).
"""
import copy
import sys
sys.path.insert(0, ".")
...
CS, ENTRY, EXIT = 0x1030, 0x4B8E, 0x4C2F
...
def main(snap="artifacts/snapshot_pre2_20260624_102733", max_calls=8):
    ...
    for _ in range(5_000_000):
        ...
        if s.cs == CS and s.ip == ENTRY:
            slots = read_particle_consume_inputs(m)[0]
            if slots:                                          # active particles this frame
    ...
    print(f"PARTICLES HOOK WHOLE-STATE: calls={res['calls']} ok={res['ok']} bad={res['bad']}")
