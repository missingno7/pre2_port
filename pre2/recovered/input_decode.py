...
from pre2.views.dgroup_view import PlayerGlobals, WidthContractBackend

MODE = 0x2879        # input source: 0=live keyboard, 1=demo playback, 2=record [dgroup_view PlayerGlobals.input_source]
DEMO_PTR = 0x287A    # demo buffer cursor (word; entries are 2 bytes at DS:[ptr+0x3F]) [PlayerGlobals.demo_ptr]
DEMO_BYTE = 0x287C   # current packed input byte (playback) [PlayerGlobals.demo_byte]
DEMO_CNT = 0x287D    # remaining repeat count for DEMO_BYTE (playback) [PlayerGlobals.demo_cnt]
DEMO_FLAG = 0x6BE5   # set on a pending demo flag (PENDING_KEY) or the 0x55AA end sentinel [PlayerGlobals.end_signal]
PENDING_KEY = 0x2874   # the DC1 pending make-code / demo any-key latch [asm 0DD6] [PlayerGlobals.pending_key]
DEMO_HDR_LEVEL = 0x83E   # demo header: level id ([0x2D8A]) stamped at ptr==0 [PlayerGlobals.attract_level]
LEVEL = 0x2D8A          # [PlayerGlobals.level]
...
JOYSTICK_CFG = 0x27E4    # bit7 set => joystick absent [asm 0D6C..0D72] [PlayerGlobals.joystick_cfg]
JOYSTICK_DISABLE = 0x27D9  # nonzero => joystick absent [asm 0D74..0D79] [PlayerGlobals.joystick_disable]
...
def _joystick_absent(g) -> bool:
    ...
    return (g.joystick_cfg & 0x80) != 0 or g.joystick_disable != 0
...
def decode_input(rb, rw):
    ...
    mode = g.input_source
    ptr = g.demo_ptr                         # local cursor; head + tail both update it (no read-after-write)

    # --- demo-decode head: modes 1 and 2 (mode 0 jumps past it) -------------- [asm 0DCF je 0xE51]
    if mode != 0:
        if g.pending_key != 0:                     # [asm 0DD6..0DDD]
            writes[DEMO_FLAG] = (1, 1)
        if ptr != 0 and g.demo_cnt != 0:     # reuse the current byte [asm 0DE6/0DEA jne 0xE0E]
    ...
    if not _joystick_absent(g):               # [asm 0E6F call 0xD6B; jb skips the joystick block]
    ...
    if ptr == 0:                             # first entry: stamp the level header, then a new entry [asm 0F48]
        writes[DEMO_HDR_LEVEL] = (g.level, 1)
