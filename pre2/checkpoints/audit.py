...
_NOT_SEPARATELY_HOOKED = [
    ...
    ("player FSM handler: eating", "1030:5F96 (=override 5F93)", "recovered / shadow-only", "player_state_eating: audio (play_sfx 0x282) + projectile spawn (0x4F2E) + render-sprite; shadow-verified 100/100 (L1) + 88/88 (L6), sfx matched; 5F93 override = same body with al=[0x4F27]"),
    ("player FSM step (full)", "1030:58A7..5A0B", "recovered / shadow-only", "player_fsm_step: front-end -> select_anim_id -> dispatch (incl eating 3/6/7 + override). Only remaining gap = idle anim13 (5D8A). Full-step standalone shadow shows perturbation-class residuals on the STATEFUL eating seq -> verify-mode at the live collapse is the authority"),
