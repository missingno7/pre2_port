...
_CATEGORY = {
    ...
    "player_fsm": "live",                     # 58A7 input->FSM front-end + state-select + handler dispatch
                                              #      (verify 28/28 clean; idle look-around anim-0x13 camera-pan
                                              #       @5D8A -> 3414/3435/350C now recovered -> crash-free live)
...
_NOT_SEPARATELY_HOOKED = [
    ...
    ("secondary list: projectiles 0x4F2E", "1030:6210", "ASM / not recovered", "remaining update-pass leaf: 4-slot anim-script walker with per-slot handler dispatch call [bx+0x79EC]"),
