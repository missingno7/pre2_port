"""Gap #3 lifecycle proof: the WHOLE game lifecycle — a full playthrough across EVERY level-end / respawn /
teleport / game-over transition — runs gameplay on the offset-free OBJECT GRAPH, byte-exact vs the recorded
VM digest.

Mirrors scripts/verify_hybrid_finish.py (the FieldBackend whole-lifecycle proof) one level further: gameplay
runs on DataclassBackend (real Player/Actor/Camera/... dataclasses); at a transition (native_level_end /
exit-anim / respawn / teleport / game-over — routines that mix raw ``.data`` access with named state) fold the
object graph into the image (``enter_image_mode``), run the transition on the plain image, then RE-SEED the
object graph from the new image (``to_object_store``). Each tick's digest is checked against the original VM
recording after materialising. If it reproduces the whole run — the earlier corpus proof only covered PURE
per-frame ticks and stopped at the first transition — the object graph is a sufficient state of record for the
ENTIRE product lifecycle, transitions included.

    python scripts/verify_object_finish.py [demo_dir] [max_ticks]
"""
import sys
...
from pre2.bridge.object_runtime import enter_image_mode, materialize, to_object_store
...
def main() -> int:
    ...
    print(f"loaded {gtd.n_ticks} recorded ticks; verifying the OBJECT-GRAPH run across ALL transitions ...")
    ...
    while i < min(max_ticks, gtd.n_ticks):
        ...
        try:
            native_gameplay_frame(state)                            # on the object graph
            ...
            enter_image_mode(state)                                   # fold the object graph into the image for the load
            ...
            to_object_store(state)                                          # re-seed the object graph from the new level
            ...
            print(f"  GAME-COMPLETE @ tick {i}: the object-graph run reached THE END after "
                  f"{i - stage_start + 1} ticks in the final stage")
            print("  PASS: the object graph reproduced the WHOLE playthrough to the ending.")
            return 0
        except Pre2HybridGap as e:
        ...
        if got != gtd.digests[i]:
            ...
            return 1
        if i % 200 == 0:
            print(f"    ... tick {i} OK (level {state.data[DS+0x2D8A]}) — on the object graph")
        i += 1

    print(f"  PASS: the object graph reproduced all {i} ticks across every stage (level {state.data[DS+0x2D8A]}).")
    return 0
